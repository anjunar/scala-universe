...
class ResolvedClass(val underlying : Type) extends Annotated {
  ...
  lazy val declaredMethods : Array[ResolvedMethod] =
    raw.getDeclaredMethods
      .filterNot(method => method.isBridge || method.isSynthetic)
      .map(method => new ResolvedMethod(method, this))
  ...
  def findDeclaredField(name : String) : ResolvedField =
    try {
      ...
      declaredFields.find(resolvedField => resolvedField.underlying == field).orNull
    } catch {
